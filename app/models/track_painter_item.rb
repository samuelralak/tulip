class TrackPainterItem < ActiveRecord::Base
	...
	private
		def update_weekly_total
			track_painters = TrackPainter.where(id: self.track_painter_id)

			unless track_painters.empty?
				track_painter = TrackPainter.find(self.track_painter_id)
				weekly_total = track_painter.track_painter_items.sum(:daily_allowance)+track_painter.track_painter_items.sum(:daily_wage)
