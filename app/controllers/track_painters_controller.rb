class TrackPaintersController < ApplicationController
  ...
  def index
    ...
    query = "SELECT COUNT(*) as count, t.week_number as week_number, t.weekly_total as weekly_total, p.name as name FROM track_painters AS t LEFT JOIN painters AS p 
      ON t.painter_id = p.id GROUP BY p.name, t.week_number, t.weekly_total HAVING COUNT(*) > 1"
