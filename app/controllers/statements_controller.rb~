class StatementsController < ApplicationController
  ...
  def show
    ...
    painter_ids = TrackPainter.where('id IN (?)', track_painter_ids).pluck(:painter_id).count
    ...
    @painters =@site.track_painter_items.pluck(:track_painter_id).inject(Hash.new(0)) { |h, e| h[e] += 1 ; h }
    @painterstwo =@painters.track_painter_items.pluck(:track_painter_id).inject(Hash.new(0)) { |h, e| h[e] += 1 ; h }
